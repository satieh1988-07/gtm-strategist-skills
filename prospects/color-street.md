...
| Corporate visibility into rep inventory — sales pushed to back office for coaching | 🔴 High | All rep inventory sales are pushed back to the DSO's back office in real time. Multi-layered value: (1) throwback campaigns and expiration tracking; (2) **proactive rep coaching** — the DSO can see how much inventory each rep holds and how fast they're selling, identify reps with stuck inventory, and reach out with coaching before the rep churns or the product expires. This transforms corporate field ops from reactive (reps call when they have a problem) to proactive (DSO identifies and intervenes before the problem surfaces). |
...
| Rep-to-rep wholesale inventory transfers | 🔴 High | Brick confirmed this is a constant, common workflow — especially when inventory is running low. **Confirmed use case:** Rep A has inventory but can't sell it; Rep B has a buyer lined up → Rep A transfers inventory to Rep B through Wayroo. Wayroo formalizes what is already happening informally (untracked, untaxed side deals between reps). Formalizing transfers in Wayroo means every transfer is tracked, taxed, and visible to corporate. Brick called the inventory management and transfer features collectively a **"massive business tool"** for reps — strongest single feature endorsement in the call. |
...
| **Prospect clarity gap: POS replacing Square and Venmo** | Color Street left the demo without a clear understanding of how Wayroo's POS fully replaces Square and Venmo for in-person and informal payments. This is not a product gap — the capability exists. It is a demo coverage gap. Must be explicitly addressed before or during the technical scoping call: show the in-person checkout flow end to end, confirm Wayroo processes card payments at events (no Square needed), and address how Venmo/cash transactions are replaced by Wayroo's guest checkout + payment link flows. If Edmond doesn't understand this, the payment separation blocker cannot be resolved — he needs to know Wayroo IS the payment processor, not an addition alongside Square. | Autumn Fowers / Cassie Lewis | Open — address in next call |
...
> *[On inventory management and rep-to-rep transfer features collectively]* **"Massive business tool."** — Brick B (Brian), Color Street (strongest single feature endorsement of the call)
